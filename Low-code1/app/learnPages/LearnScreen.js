...
const LearnScreen = () => {
  ...
  const [fusionAnimationVisible, setFusionAnimationVisible] = useState(false);
  const fusionAnimRef = useRef(null);
    ...
    const checkFusion = (list, setList, zoneName) => {
      ...
      Object.keys(counts).forEach((mol) => {
        if (!mol.startsWith('F-') && counts[mol] >= 2) {
          ...
          triggerHaptic();
          playDropSound();
          setFusionAnimationVisible(true);
          fusionAnimRef.current?.play();
          setTimeout(() => setFusionAnimationVisible(false), 1500);
        }
      });
    };
  ...
  return (
    <View style={styles.container}>
      ...
      {fusionAnimationVisible && (
        <View style={styles.lottieContainer}>
          <LottieView ref={fusionAnimRef} style={styles.lottie} />
        </View>
      )}
    </View>
  );
